...
if __name__ == '__main__':
    ...
    data_9=pd.DataFrame()
    for p in paths:
        path='../data/tcdata/disk_sample_smart_log_round2/'+p
...
# 'smart_1raw','smart_1_normalized','smart_190_normalized','model']]
    ...
    with open('../model/tag_model.pkl', 'rb') as fr:
    ...
    best_iter=[58,59]
    pred=0
    for index,b_t in enumerate(best_iter):
        with open('../model/ensenble_lgb{}_{}.pkl'.format(index,b_t), 'rb') as fr:
            clf = pickle.load(fr)
  ...
  ##### 最新策略评估-----------------
    ...
    final_result.loc[:, ['manufacturer','model','serial_number','dt']].to_csv('../prediction_result/result.csv', index=False, header=False)
    os.system('zip result.zip ../prediction_result/result.csv')
    #---------------------------------------
